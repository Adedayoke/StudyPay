...
export function PaymentConfirmation({ 
  ...
  // Device detection for dual implementation (consistent with other flows)
  const isMobile = typeof window !== 'undefined' && (
    /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ||
    (window.innerWidth <= 768 && 'ontouchstart' in window)
  );

  // Choose payment method based on device (consistent with other components)
  ...
  const handleConfirmPayment = () => {
    if (useSolanaPay) {
      // 📱 Mobile: Open Solana Pay URL in external wallet (consistent with other flows)
